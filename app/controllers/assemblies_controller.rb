class AssembliesController < ApplicationController
  ...
  def show
    ...
    parts = Part.select('id, title').all
    assemblies = Assembly.select('id, title').where('id <> ?', @assembly.id)

    @grouped_options = {
      Part.model_name.human.pluralize => parts.collect {|i| [i.title, i.id, {'data-assemblable-type' => i.class.to_s}]},
      Assembly.model_name.human.pluralize => assemblies.collect {|i| [i.title, i.id, {'data-assemblable-type' => i.class.to_s}]}
    }

    @new_item = AssemblyItem.new :assembly => @assembly
